...
class BotState:
    def __init__(self, config) -> None:
        ...
        self.recent_kick_timestamps: Dict[int, Union[float, datetime]] = {}  # Changed to Union type
    ...
    def clean_old_entries(self) -> None:
        """Unified cleanup maintaining 24h history and 100-entry limits"""
        ...
        self.recent_kick_timestamps = {
            ...
            if current_time - (v.timestamp() if isinstance(v, datetime) else v) < 86400
        }
        
        # Clean lists with 24h/100-entry rules
        list_specs = {
            'recent_joins': (3, 100),
            'recent_leaves': (3, 100),
            'recent_bans': (3, 100),
        ...
        for list_name, (time_idx, max_entries) in list_specs.items():
            ...
            cleaned = [
                ...
                if len(entry) > time_idx and (now - entry[time_idx]).total_seconds() < 86400
            ][-max_entries:]
        ...
        if len(self.recently_logged_commands) > 100:
